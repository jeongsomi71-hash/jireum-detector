...
class AdvancedSearchEngine:
    ...
    def categorize_deals(titles, search_query, user_excludes):
        ...
        for text in titles:
            ...
            if num < 10000: continue # 1만원 미만 부품가 원천 차단
        ...
        # [v2.6 신규] 가격 역전 방지 로직 (Cross-Check)
        # 예: 10인용의 최저가가 6인용 최저가의 80% 미만일 경우, 10인용 데이터의 최하단 이상치 제거
        if "10인용" in cleaned_cat and "6인용" in cleaned_cat:
            if cleaned_cat["10인용"][0] < cleaned_cat["6인용"][0] * 0.8:
                if len(cleaned_cat["10인용"]) > 1:
                    cleaned_cat["10인용"].pop(0)
...
# ==========================================
def apply_style():
    st.set_page_config(page_title="지름신 판독기 PRO v2.6", layout="centered")
    st.markdown("""
        <style>
        ...
        .link-btn-box { background-color: #333333 !important; color: #FFFFFF !important; padding: 10px; border-radius: 8px; text-align: center; font-size: 0.85rem; border: 1px solid #FFFFFF !important; font-weight: bold; margin-bottom: 5px; }
        .review-btn-box { background-color: #004d40 !important; color: #00FF88 !important; padding: 10px; border-radius: 8px; text-align: center; font-size: 0.85rem; border: 1px solid #00FF88 !important; font-weight: bold; }
...
def main():
    ...
    # [오류해결] Session State 초기값 설정 (rerun 후에도 유지)
    ...
    st.markdown('<div class="unified-header">⚖️ 지름신 판독기 PRO <span style="font-size:0.8rem; color:#444;">v2.6</span></div>', unsafe_allow_html=True)
    ...
    with c1:
        if st.button("🔍 시세 판독 실행"):
            if name_input:
                st.session_state.p_name = name_input
                st.session_state.p_price = price_input
                st.session_state.p_exclude = exclude_input
                with st.spinner('🏘️ 역전현상 검증 및 분석 중...'):
    ...
    with c2:
        if st.button("🔄 리셋"):
            # [오류해결] 직접 할당 대신 clear 및 rerun으로 안전하게 초기화
    ...
    if st.session_state.results:
        ...
        st.markdown("### 📊 분석 리포트 (역전현상 검증완료)")
        
        for key, prices in sorted(cat_data.items(), reverse=True): # 상위 모델부터 표시
            min_p = prices[0]
            count = len(prices)
            ...
            <div class="detail-card">
                <span style="color:{("#00FF88" if count>=4 else "#FF5555")}; font-weight:bold; font-size:0.8rem;">{rel_txt} (표본 {count}건)</span><br>
        ...
        with l_col:
            st.markdown(f'<a href="https://m.ppomppu.co.kr/new/search_result.php?search_type=sub_memo&keyword={e_q}&category=1" target="_blank" style="text-decoration:none;"><div class="link-btn-box">뽐뿌 시세</div></a>', unsafe_allow_html=True)
            st.markdown(f'<a href="https://www.clien.net/service/search?q={e_q}" target="_blank" style="text-decoration:none;"><div class="link-btn-box">클리앙 시세</div></a>', unsafe_allow_html=True)
        with r_col:
            st.markdown(f'<a href="https://www.ppomppu.co.kr/zboard/zboard.php?id=free_picture&category=2&keyword={e_q}" target="_blank" style="text-decoration:none;"><div class="review-btn-box">뽐뿌 사용기</div></a>', unsafe_allow_html=True)
            st.markdown(f'<a href="https://www.clien.net/service/search/board/use?sk=title&sv={e_q}" target="_blank" style="text-decoration:none;"><div class="review-btn-box">클리앙 리뷰</div></a>', unsafe_allow_html=True)

if __name__ == "__main__": main()

# Version: v2.6 - Fixed SessionState Error, Added Cross-Category Validation (Anti-Reversal)